...
class EncoderDecoder1dMModel(BaseModel):
    
    def __init__(self, input_nc, output_nc, lr, isTrain):
        BaseModel.__init__(self)
#        self.gpu_ids = str(0)
#        self.device = torch.device('cuda:{}'.format(self.gpu_ids[0])) if self.gpu_ids else torch.device('cpu')
        self.save_dir = './checkpoints/encoderdecoder1dM/'
        self.loss_names = ['L1','L2']
        self.model_names = ['G']
        self.visual_names = ['real_A', 'fake_B', 'real_B', 'real_M']
        
#        self.netG = networks.define_M_EncoderDecoder(input_nc, output_nc, init_type='normal', init_gain=0.02, gpu_ids = self.gpu_ids, use_norm = True)
...
#        self.netG = networks.define_ShortM_Encoder_v3(input_nc, output_nc, init_type='normal', init_gain=0.02, gpu_ids = self.gpu_ids, use_norm = True)
